...
## 直近の作業 (2026-08-26時点) — Physical AI / VSS の産業リサーチを docs/ に追加

「VSS(動画検索・要約) × Physical AI × Autonomous Industry の構造変化から、日本の中小型株を
発掘したい」という調査依頼を受けて、`docs/research/vss-physical-ai-japan-2035.md` を新規作成。
**アプリのコードは一切触っていない**(リサーチ成果物のみ)。

- 構成: 産業構造変化10項目 → Autonomous Industry 産業ランキング15 → 技術スタック(L0〜L7) →
  将来ボトルネック → 利益プール → **50社ランキング** → 上位10社の詳細Thesis(23項目) →
  11〜25位の要約 → Mispricing記述 → Bear反論8つ → 中国競争 → メタトレンド10 → 次のアクション
- 中核の結論: **「観測する手段」と「作用する手段」を同一社内に持つ企業だけが閉ループを閉じられる**。
  上位はニレコ(6863) / 技研製作所(6289) / 横河電機(6841) / 日本信号(6741) / オークマ(6103)
- 実際にWebで数値を確認できたのは**50社中17社のみ**。残りは「要検証」と明記して推測値は書いていない
  (株価情報サイトがこの環境からアクセス遮断されているため)
- 調査中に **トプコン(7732)は2025/12上場廃止、パスコ(9232)は2025/1上場廃止**と判明したため、
  本テーマの最有力候補だったが除外した。証券コードは各自で再照合が必要

### 未完了(ユーザー側での確認が必要)

上位10社の有価証券報告書・中期経営計画・決算説明会書き起こしの精読が未了。
「経営陣が本当にソフトウェア/データ事業をやる気があるか」がこの投資仮説の成否を決めるので、
レポートの「⑫ 次のアクション」に確認すべき項目を列挙してある。
