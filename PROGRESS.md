...
## 直近の作業 (2026-09-03時点・続き) — Bring List を本当の共有リストにした(**要SQL実行**)
...
- `supabase/migrations/0034_bring_list.sql`(新規・**未実行**)
...
- ブラウザ自動操作で **52項目PASS**(ローカル動作は全部そのまま。加えて
  「共有に失敗してもページが壊れない」「壊れたリンクは無視する」「合言葉は24文字」も確認)
- **クラウド側は一切未検証**。この作業環境は jsdelivr が塞がれていて `supabase` が読めず、
  DBにも繋げないため。**SQLを実行したあとに実機で確認が必要**:
  1. Supabase の SQL Editor で `0034_bring_list.sql` を実行
  2. アプリで Create a shared link → 別ブラウザ(シークレット等)でそのURLを開く
  3. 片方で claim → もう片方に数秒で反映されるか
  4. **合言葉を1文字変えたURLでは何も見えないこと**(RLSが効いている証拠)
- SQL未実行のままでも local モードは普通に動く。共有ボタンを押すと
  「まだ用意ができていないかも」という説明が画面に出るだけ
