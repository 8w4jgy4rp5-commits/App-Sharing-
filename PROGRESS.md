...
## 直近の作業 (2026-09-18) — 新規ミニアプリ `trip-budget` を追加（51個目）

CobbleWorksのリクエスト「旅行の予算を決めても、いつも使いすぎてしまう」に対して作ったアプリ。
カテゴリごとに上限を決めて、旅の途中で支出を記録していく1画面。

### 中身

- `apps/trip-budget/`（index.html / style.css / script.js）と `app-icons.js` に1行（緑 `c1`・紙幣の絵）
- 見た目は3案（Ledger / CobbleWorks / Boarding Pass）を並べて見せて、**1. Ledger** を選んでもらった。
  等幅フォント・角ゼロ・細い罫線で、数字が縦に揃う台帳風
- 保存は `AppSync` を2キーに分割: `trip`（旅行名・通貨・カテゴリ）と `expenses`（支出）。
  丸ごと置き換えの同期なので、キーを分けると2台同時編集で失う範囲が狭くなる
- 機能: カテゴリ追加/編集/削除（削除時はぶら下がる支出も一緒に消す確認つき）、支出の記録と削除、
  カテゴリ別バー（超過は赤 + Over by 表示）、全体の合計、通貨記号5種、空状態、How to use

### 途中で直した別の問題（`tools/seo.js`）

`node tools/seo.js` を実行したら、**apps.html からPWAのタグとService Worker登録が消え、
sitemap.xml から about.html / submit.html が落ちた**。
原因は、apps.html は seo.js が毎回まるごと生成しているのに、2026-09-13のPWA化で
apps.html に手で足したタグが seo.js 側のテンプレートに反映されていなかったこと。
sitemap も seo.js が知っているページ一覧に about/submit が入っていなかった。
出力を手で直すと次回の実行でまた消えるので、**seo.js のテンプレートとページ一覧の方を直した**。

### 確認したこと

`node test/run.js` 463 passed / 0 failed、`node tools/seo.js` 実行済み。
375px幅のiframe方式で、空状態・データあり・アイコン40pxを目視確認。
操作もiframe経由で自動実行し、34項目（バリデーション・追加・編集・削除・リロード後も残る・
保存キー・コンソールエラー無し）すべてOK。

### 次の小さな一歩

投稿フォームへの登録（下の投稿用テキストを貼るだけ）。その後に欲しくなりそうなのは、
旅行を複数持てるようにすること（今は1つだけ）。
