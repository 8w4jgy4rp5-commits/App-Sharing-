...
- プラットフォーム機能: いいねバッジ + Mini Apps一覧の並び替えタブを追加。push・commit済み。
  - 新規テーブル `likes`(`supabase/migrations/0024_likes.sql`・**未実行**)。1ユーザー1アプリ1いいね
  - 合計いいね数でバッジ判定: 銅5・銀20・金50(しきい値は仮、ユーザーの希望で後で調整予定)
  - Mini Apps一覧上部に並び替えタブ New/Trending/Viral/Popular を追加
  ...
  - **未完了(ユーザー側の手作業が必要)**: `0024_likes.sql`をSupabaseのSQL Editorで実行するまで、
