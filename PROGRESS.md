...
## 直近の作業 (2026-08-10時点) — 米国株スクリーニングの下準備

- 日本株の`cycle-value-screen`の結論(現時点ではシクリカルバリューは見送りが妥当)を受けて、同じ手法を米国株に適用する方針に。**このセッションでは業界の絞り込みと論点整理までを実施**(個社の定量分析は次回)

### 【重要・次セッションで最初に確認】ネットワーク設定の追加依頼
- 米国の財務データサイトは現在すべて`EGRESS_BLOCKED`(curl・WebFetchとも403)。許可されているのは前回追加した日本の3ドメイン(irbank.net/kabutan.jp/minkabu.jp)のみ
- ユーザーに以下の追加を依頼済み: `stockanalysis.com` / `data.sec.gov` / `www.sec.gov` / `finviz.com` / `macrotrends.net`
- `data.sec.gov`はSEC公式のXBRL API(ボット判定なし・最も正確)。二次データ(stockanalysis)との突き合わせ体制を作る狙い
- 次セッションの最初に疎通テストすること(例: `https://data.sec.gov/api/xbrl/companyconcept/CIK0000320193/us-gaap/NetIncomeLoss.json`)。まだブロックされていれば、環境が正しく切り替わっているかをユーザーに確認

### 判明した最重要の論点: 日米でサイクルの位相が逆
- 日本: FY26/3で回復転換(自動車部品の減益社比率 74%→34%、赤字社7→4社)
- 米国: ディストレス比率が2026年3月に7.23%へ上昇、6か月連続で悪化
- → 日本で「もう遅い」と判断した理由が、米国では「まだ渦中」を意味する可能性

### 候補業界の絞り込み結果
1. **トラック運送(最有力)** — 日本で見つからなかった条件が揃っている
   - 約3年の運賃不況、truckload平均営業利益率-2.3%(走るほど赤字)、Werner(WERN)は直近12ヶ月EPS -$0.77
   - 10万超の事業者が退出(2025年だけで5,000〜8,000社)。走行コストは$2.26/mile とATRI観測史上最高
   - 2026年に運賃急反発: スポット$2.96/mile(3月・YoY+27%)、6月は燃料除きYoY+43%、契約運賃+13%
   - **需要回復ではなく供給の破壊による反発**(出荷量はまだYoY-4.5%)＝古典的なシクリカル・バリューの形
   - トラック・ターミナル・不動産を持ち資産バリューの検証が可能。上場20〜25社
   - ⚠ 既に株価が動いている懸念: Covenant(CVLG)は1年で+48%、Wernerは「楽観的利益で11〜12倍」と割高視する見方あり
2. **化学** — 日本は回復済み(2期連続赤字0社)だが米国はまだ底にいない。2022年からのダウンサイクル継続、BASF/Dow/Eastmanが数十億ドルのコスト削減・工場閉鎖。ただし2026年に中国・米国で800万トン超のポリエチレン新設＝構造的過剰供給が悪化方向で、循環回復のシナリオが描きにくい
3. **対象外と判断**: ソフトウェア/テック(ディストレス債の45%だが資産価値がなく循環的でもない)、再生可能エネルギー(Chapter 11継続中・慢性赤字で財務脆弱。日本の創薬バイオと同じ「赤字だが割安ではない」パターンの懸念)

### 次セッションの検証項目(トラック運送)
核心は「もう遅いのか、まだ間に合うのか」の1点。必要な定量データ:
1. 20〜25社の直近5期の純利益(2期連続赤字が何社あるか)
2. 現在のPBR・純現金・自己資本比率(資産バリューが本物か)
3. **EV/純利益での検算** — トラック運送はリース・車両ローンで負債が重く、日本の自動車部品と同じ「借金で安く見える」罠が濃厚
4. 株価の52週レンジ(どこまで織り込み済みか)
