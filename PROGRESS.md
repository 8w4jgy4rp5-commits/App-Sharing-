...
## 直近の作業 (2026-08-24時点) — AIデータセンター×水インフラの投資リサーチを追加

アプリ開発ではなく調査タスク。`docs/research/ai-datacenter-water-infrastructure-2026-08.md` を新規作成。
「AI DC増加 → 冷却需要増 → 水需要増 → 水インフラ投資」という因果連鎖を一次情報で検証した結果、
**4番目のリンク（冷却需要増 → 水需要増）が成立していない**ことが最大の発見。
- Microsoftは2024年8月以降の全新設設計がゼロ水蒸発、AWSのWUEは0.12 L/kWh（年▲20%）
- Xylemはデータセンター売上が+200%成長しても全社売上の約2%にしか届かない
- 一方、発電由来の間接水は直接水の約12倍（LBNL）で、ここは誰も削減していない
結論は「弱気寄りの中立」。Tier 1はオルガノ/SPX/Ecolab/Munters/Vertiv。
アプリのコードには一切触れていない。
