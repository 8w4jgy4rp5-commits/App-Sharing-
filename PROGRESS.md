...
## 直近の作業 (2026-08-18時点) — Thread Composer の取りこぼし回収

アイコンのPR(#8)は8/17にマージ済み。その後に入った Thread Composer(PR #10)が
**2つの「アプリ追加時にやること」から漏れていた**ので回収した。

| 漏れていたもの | 症状 | 検知 |
|---|---|---|
| `app-icons.js` への追加 | 一覧で1つだけ頭文字`T`のバッジ | `test/app-icons.test.js` が落ちて検知 |
| `node tools/seo.js` の再実行 | `apps.html`・`sitemap.xml` に載らず、description/OGP/戻りリンクも無し | 手動で気づいた |

- アイコン: 線でつないだ3つの点(=スレッド)。`message-writer`(吹き出し)と同じ濃茶タイルだが絵は別物
- `tools/seo.js` を再実行。差分は Thread Composer 分だけで、他36アプリには一切churnが出ないことを確認
  (冪等性が保たれている)。`apps.html` の件数表記も 36→37 に自動更新された

**アプリを新規追加したときのチェックリスト**(この2つは忘れやすい):

1. `app-icons.js` にアイコンを足す ← 忘れるとテストが落ちるので気づける
2. `node tools/seo.js` を実行する ← **今のところ自動検知が無い。忘れると検索に載らない**

2 も `apps.html` の件数と `apps/` の数を突き合わせるテストを足せば検知できる。次にやるなら候補。
