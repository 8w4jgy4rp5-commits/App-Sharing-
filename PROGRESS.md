...
## 直近の作業 (2026-08-09時点・続き2)

- `cycle-value-screen`スキルの2回目の実行。テーマは「業界全体が不況(株価低迷×2年連続赤字が多い)の業種を探す」。**結論: 条件を満たす業界は存在しなかった**
- 5業界138社を irbank.net + kabutan.jp の一次データでスクリーニング(自動車部品35・化学34・繊維アパレル29・紙パルプ10・創薬バイオ30)
  - 自動車部品: 2期連続赤字 2社(6%) / PBR0.7倍未満 18社(51%) / PBR中央値0.64
  - 紙パルプ: 0社(0%) / 8社(80%) / 0.59
  - 化学: 0社(0%) / 2社(6%) / 0.99
  - 繊維・アパレル: 1社(3%) / 10社(34%) / 1.12
  - 創薬バイオ: 23社(77%) / 1社(3%) / 4.42
  - → 「安い業界は黒字」「赤字だらけの業界は割高」で2条件が排他的。FY26/3が広範な回復年だったことと、東証のPBR改善要請3年目でディープバリューの母集団自体が縮小(プライムのPBR1倍割れ比率は約20%と2018年以来の低水準)していることが背景
- **重要な分析**: 低PBR自動車部品の割安感は有利子負債で相殺される。EV(時価総額+有利子負債-現金)ベースで検算すると エフテック PER2.9倍→10.5倍、ミクニ 10.2→34.5、曙ブレーキ 15.9→35.1 と逆転。純現金を持つ東海理化・トヨタ紡織・日本精機・エクセディは逆にEVで安くなる
- 収益バリュー/資産バリューの切り分け: 資産バリュー=東海理化・日本精機・トヨタ紡織・パイオラックス / 収益バリュー=エイチワン・アーレスティ・中央発條・ミツバ / バリュートラップ懸念=ファルテック・ミクニ・河西工業・TBK
- アーティファクト: https://claude.ai/code/artifact/0f39ce3d-bf80-475e-90cf-537d5e1253ef
- **自作スクリプトの不具合を3件検出・修正(教訓として記録)**:
  1. kabutanのPER欄が「－」(赤字企業)だとPBR欄が1つずれ信用倍率を拾う → 探索対象である赤字企業ほど誤値が出る
  2. **IFRS採用企業はirbankの列構成が日本基準と異なり(経常利益がない)、包括利益を純利益として誤取得** → エイチワン純利益192.8億→109.7億、日本精機196億→82億、三菱ケミカルG 2519億→118億。列位置ではなく列見出しで拾うよう修正
  3. 列見出しの脚注記号(`当期純利益#1`、`売上収益#1#2`)で完全一致に失敗
  - 前回の1/10スケール誤りと同種。**大量銘柄の自動処理では、必ず数社を個別に一次情報と突き合わせて検算すること**
- 副次的に判明した誤り: ティッカー8218はコナカではなくコメリ(コナカは7494)
