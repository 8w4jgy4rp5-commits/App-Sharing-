...
const LEGACY_STORAGE_KEY = 'freeTrialTracker:trials:v1';
...
// store.get() は毎回コピーを返すので、結果をそのまま書き換えて saveTrials() してよい。
function getTrials() {
  if (!store) return [];
  const v = store.get();
...
function saveTrials(trials) {
  if (!store) return;
  store.set(trials).catch(function (e) {
    console.error('Free Trial Tracker: 保存に失敗しました', e);
  });
}
...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  store = await openStore('free-trial-tracker', 'trials', {
    default: [],
    legacyKey: LEGACY_STORAGE_KEY
  });

  // subscribe は他デバイス・他タブ由来の変更でしか呼ばれない
  store.subscribe(function () { renderTrialList(); });
