...
const LEGACY_STORAGE_KEY = 'whatToCook:meals:v1';
...
// store.get() は毎回コピーを返すので、結果をそのまま書き換えて saveMeals() してよい。
function getMeals() {
  if (!store) return [];
  const v = store.get();
...
function saveMeals(meals) {
  if (!store) return;
  store.set(meals).catch(function (e) {
    console.error('What to Cook: 保存に失敗しました', e);
  });
}
...
// データ層の準備ができてから描画する
(async function () {
  store = await openStore('what-to-cook', 'meals', {
    default: [],
